...
class BLEService : Service() {
    ...
    private val mGattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {//Change in connection state
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {//See if we are connected
                ...
                broadcastUpdate(BleConstant.ACTION_GATT_CONNECTED)//Go broadcast an intent to say we are connected
                ...
                broadcastUpdate(BleConstant.ACTION_GATT_DISCONNECTED)//Go broadcast an intent to say we are disconnected
            }
        }
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {              //BLE service discovery complete
            if (status == BluetoothGatt.GATT_SUCCESS) {                                 //See if the service discovery was successful
                ...
                broadcastUpdate(BleConstant.ACTION_GATT_SERVICES_DISCOVERED)                       //Go broadcast an intent to say we have discovered services
        ...
        override fun onCharacteristicRead(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) { //A request to Read has completed
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                broadcastUpdate(BleConstant.ACTION_DATA_AVAILABLE, characteristic)                 //Go broadcast an intent with the characteristic data
            } else {
        ...
        override fun onCharacteristicWrite(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) { //A request to Write has completed
            if (status == BluetoothGatt.GATT_SUCCESS) {                                 //See if the write was successful
                ...
                broadcastUpdate(BleConstant.ACTION_DATA_WRITTEN, characteristic)                   //Go broadcast an intent to say we have have written data
            }
        }
    ...
    private fun broadcastUpdate(action: String, characteristic: BluetoothGattCharacteristic) {
        ...
        if (value != null) {
            ...
            intent.putExtra(BleConstant.EXTRA_DATA, dataValueNew)
            intent.putExtra(BleConstant.EXTRA_UUID, characteristic.uuid)
    ...
    private fun processIntent(intent: Intent?, isNeedToSend: Boolean) {
        if (isNeedToSend) {
            ...
            intent!!.putExtra(BleConstant.EXTRA_DATA, value)
            sendBroadcast(intent)
        }
    ...
    fun initialize(): Boolean {
        if (mBluetoothManager == null) {                                                //See if we do not already have the BluetoothManager
            mBluetoothManager = getSystemService(BLUETOOTH_SERVICE) as BluetoothManager //Get the BluetoothManager
